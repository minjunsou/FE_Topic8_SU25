...
const nurseApi = {
  ...
  getAllStudents: async () => {
    try {
      const response = await axiosInstance.get('/v1/accounts?roleId=1&size=1000');
      return response.data?.accounts || [];
    } catch (error) {
      console.error('Lỗi khi lấy danh sách học sinh:', error);
      throw error;
  ...
  verifyExternalVaccine: async (externalVaccineId) => {
    try {
      const response = await axiosInstance.put(`/external-vaccine-records/${externalVaccineId}/verify`);
      return response.data;
    } catch (error) {
      console.error(`Lỗi khi verify external vaccine ${externalVaccineId}:`, error);
      throw error;
    }
  },
  ...
  /**
   * Vaccination Statistics APIs
   */
  getVaccinationStatistics: async () => {
    try {
      const response = await axiosInstance.get('/v1/vaccination-confirmations/statistics/overview');
      return response.data || {};
    } catch (error) {
      console.error('Lỗi khi lấy thống kê tiêm chủng:', error);
      throw error;
    }
  },

  getVaccinationStatisticsByNotice: async (noticeId) => {
    try {
      const response = await axiosInstance.get(`/v1/vaccination-confirmations/statistics/${noticeId}`);
      return response.data || {};
    } catch (error) {
      console.error(`Lỗi khi lấy thống kê tiêm chủng cho notice ${noticeId}:`, error);
      throw error;
    }
  },
   ...
   * Tạo sự cố y tế mới
   * @param {string} studentId - ID của học sinh
   * @param {Object} data - Dữ liệu sự cố y tế
   * @returns {Promise} - Promise chứa thông tin sự cố đã tạo
   */
  createHealthEvent: async (studentId, data) => {
    try {
      ...
      // Validate that the current user is actually a nurse (roleId = 3)
      if (userInfo.roleId !== 3) {
        throw new Error('Người dùng hiện tại không phải là y tá');
      }

      console.log('Creating health event with:', {
        studentId,
        nurseId,
        data
      });

      const response = await axiosInstance.post(`/v1/healthEvents/create/${studentId}/${nurseId}`, data);
      console.log('Health event created:', response.data);
      ...
      if (error.response) {
        ...
        if (status === 500 && data?.message) {
          if (data.message.includes('Student not exists or not a student')) {
            ...
            throw new Error('Y tá không tồn tại hoặc không phải là y tá');
          }
        }
      }
   ...
   * @returns {Promise} - Promise chứa danh sách sự cố y tế
   */
  getAllHealthEvents: async () => {
    try {
      const response = await axiosInstance.get('/v1/healthEvents/get-all');
      console.log('All health events:', response.data);
      return response.data.data || response.data || [];
    } catch (error) {
  ...
  /**
   * Lấy sự cố y tế theo ngày
   * @param {string} eventDate - Ngày sự cố (format: YYYY-MM-DD)
   * @returns {Promise} - Promise chứa danh sách sự cố theo ngày
   */
  getHealthEventsByDate: async (eventDate) => {
    try {
      const response = await axiosInstance.get(`/v1/healthEvents/get-by-date/${eventDate}`);
      console.log(`Health events for date ${eventDate}:`, response.data);
      return response.data.data || response.data || [];
    } catch (error) {
  ...
  /**
   * Lấy sự cố y tế theo mức độ ưu tiên
   * @param {string} priority - Mức độ ưu tiên (LOW, MEDIUM, HIGH, CRITICAL)
   * @returns {Promise} - Promise chứa danh sách sự cố theo mức độ
   */
  getHealthEventsByPriority: async (priority) => {
    try {
      const response = await axiosInstance.get(`/v1/healthEvents/priority/${priority}`);
      console.log(`Health events with priority ${priority}:`, response.data);
      return response.data.data || response.data || [];
    } catch (error) {
      console.error(`Lỗi khi lấy sự cố y tế theo mức độ ${priority}:`, error);
      throw error;
    }
  },

  /**
   * Lấy sự cố y tế của y tá hiện tại
   * @returns {Promise} - Promise chứa danh sách sự cố của y tá
   */
  getHealthEventsByCurrentNurse: async () => {
    try {
      const userInfo = JSON.parse(localStorage.getItem('userInfo') || '{}');
      const nurseId = userInfo.accountId;
      ...
      console.log('Health events for current nurse:', response.data);
      return response.data.data || response.data || [];
    } catch (error) {
      console.error('Lỗi khi lấy sự cố y tế của y tá hiện tại:', error);
      throw error;
    }
  },
  ...
  /**
   * Cập nhật sự cố y tế
   * @param {number} eventId - ID của sự cố
   * @param {Object} data - Dữ liệu cập nhật
   * @returns {Promise} - Promise chứa thông tin sự cố đã cập nhật
   */
  updateHealthEvent: async (eventId, data) => {
    try {
      const response = await axiosInstance.put(`/v1/healthEvents/update/${eventId}`, data);
      console.log('Health event updated:', response.data);
      return response.data.data || response.data;
    } catch (error) {
  ...
  deleteHealthEvent: async (eventId) => {
    try {
      ...
      console.log('Health event deleted:', response.data);
      return response.data;
    } catch (error) {
  ...
  /**
   * Tìm kiếm sự cố y tế với các bộ lọc
   * @param {Object} filters - Các bộ lọc tìm kiếm
   * @returns {Promise} - Promise chứa danh sách sự cố đã lọc
   */
  searchHealthEvents: async (filters = {}) => {
    try {
      const response = await axiosInstance.get('/v1/healthEvents/search', { params: filters });
      console.log('Health events search result:', response.data);
      return response.data.data || response.data || [];
    } catch (error) {
  ...
  /**
   * Lấy thuốc sử dụng trong sự cố y tế
   * @param {number} eventId - ID của sự cố
   * @returns {Promise} - Promise chứa danh sách thuốc
   */
  getMedicationsByHealthEvent: async (eventId) => {
    try {
      const response = await axiosInstance.get(`/v1/healthEvents/${eventId}/medications`);
      console.log(`Medications for health event ${eventId}:`, response.data);
      return response.data.data || response.data || [];
    } catch (error) {
      console.error(`Lỗi khi lấy thuốc cho sự cố y tế ${eventId}:`, error);
      throw error;
    }
  },

  /**
   * Lấy sự cố y tế chờ phê duyệt của phụ huynh
   * @param {string} parentId - ID của phụ huynh
   * @returns {Promise} - Promise chứa danh sách sự cố chờ phê duyệt
   */
  getHealthEventsPendingApproval: async (parentId) => {
    try {
      const response = await axiosInstance.get(`/v1/healthEvents/parent/${parentId}/pending-approval`);
      console.log('Health events pending approval:', response.data);
      return response.data.data || response.data || [];
    } catch (error) {
      console.error(`Lỗi khi lấy sự cố chờ phê duyệt cho phụ huynh ${parentId}:`, error);
      throw error;
    }
  },
   ...
   * @returns {Promise} - Promise chứa kết quả phê duyệt
   */
  approveHealthEvent: async (approvalData) => {
    try {
      const response = await axiosInstance.put('/v1/healthEvents/approve', approvalData);
      console.log('Health event approval:', response.data);
      return response.data.data || response.data;
    } catch (error) {
      console.error('Lỗi khi phê duyệt sự cố y tế:', error);
      throw error;
    }
  },

  /**
   * Lấy thống kê sự cố y tế
   * @returns {Promise} - Promise chứa thống kê sự cố y tế
   */
  getHealthEventStatistics: async () => {
    try {
      // Tính toán thống kê từ dữ liệu health events
      const allEvents = await nurseApi.getAllHealthEvents();
      
      const statistics = {
        total: allEvents.length,
        pending: allEvents.filter(event => event.parentApprovalStatus === 'PENDING').length,
        completed: allEvents.filter(event => event.status === 'COMPLETED').length,
        critical: allEvents.filter(event => event.priority === 'CRITICAL').length,
        high: allEvents.filter(event => event.priority === 'HIGH').length,
        medium: allEvents.filter(event => event.priority === 'MEDIUM').length,
        low: allEvents.filter(event => event.priority === 'LOW').length,
        requiresHomeCare: allEvents.filter(event => event.requiresHomeCare === true).length
      };
      
      console.log('Health event statistics:', statistics);
      return statistics;
    } catch (error) {
  ...
  /**
   * Tạo follow-up cho sự cố y tế
   * @param {Object} data - Dữ liệu follow-up
   * @returns {Promise} - Promise chứa thông tin follow-up đã tạo
   */
  createHealthEventFollowUp: async (data) => {
    try {
      const response = await axiosInstance.post('/v1/health-event-followups', data);
      console.log('Health event follow-up created:', response.data);
      return response.data;
    } catch (error) {
      console.error('Lỗi khi tạo follow-up sự cố y tế:', error);
      throw error;
    }
  },

  /**
   * Lấy follow-ups của sự cố y tế
   * @param {number} eventId - ID của sự cố
   * @returns {Promise} - Promise chứa danh sách follow-ups
   */
  getFollowUpsByHealthEvent: async (eventId) => {
    try {
      const response = await axiosInstance.get(`/v1/health-event-followups/event/${eventId}`);
      console.log(`Follow-ups for health event ${eventId}:`, response.data);
      return response.data || [];
    } catch (error) {
      console.error(`Lỗi khi lấy follow-ups cho sự cố y tế ${eventId}:`, error);
      throw error;
    }
  },
   ...
   * @param {number} followUpId - ID của follow-up
   * @param {string} status - Trạng thái mới
   * @returns {Promise} - Promise chứa thông tin follow-up đã cập nhật
   */
  updateFollowUpStatus: async (followUpId, status) => {
    try {
      const response = await axiosInstance.put(`/v1/health-event-followups/${followUpId}/status?status=${status}`);
      console.log('Follow-up status updated:', response.data);
      return response.data;
    } catch (error) {
      console.error(`Lỗi khi cập nhật trạng thái follow-up ${followUpId}:`, error);
      throw error;
    }
  },
   ...
   * Xác nhận follow-up
   * @param {number} followUpId - ID của follow-up
   * @returns {Promise} - Promise chứa thông tin follow-up đã xác nhận
   */
  acknowledgeFollowUp: async (followUpId) => {
    try {
      const response = await axiosInstance.put(`/v1/health-event-followups/${followUpId}/acknowledge`);
      console.log('Follow-up acknowledged:', response.data);
      return response.data;
    } catch (error) {
      console.error(`Lỗi khi xác nhận follow-up ${followUpId}:`, error);
      throw error;
    }
  },
  ...
  getStudents: async () => {
    try {
      const response = await axiosInstance.get('/v1/accounts/students');
   ...
   * @returns {Promise} - Promise chứa thông tin y tá
   */
  getCurrentNurse: async () => {
    try {
      ...
      const response = await axiosInstance.get(`/v1/accounts/${nurseId}`);
      console.log('Current nurse data:', response.data);
      return response.data.data || response.data;
    } catch (error) {
      console.error('Lỗi khi lấy thông tin y tá:', error);
      throw error;
    }
  },
};
